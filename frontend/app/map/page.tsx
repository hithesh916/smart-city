...
export default function MapPage() {
  ...
  const fetchMapData = useDebouncedCallback(async (bounds: any) => {
    ...
    try {
      ...
      const [trafficRes, summaryRes] = await Promise.all([
        ...
        fetch(`http://localhost:8001/api/analytics/summary?${params}`),
      ]);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8001/api/geocode/search?q=${encodeURIComponent(
          searchQuery
        )}`
      );
  ...
  const handleProbeUpdate = async (lat: number, lng: number) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8001/api/probe/analyze?lat=${lat}&lng=${lng}`
      );
      if (res.ok) {
        const data = await res.json();
        setSummary(data);
